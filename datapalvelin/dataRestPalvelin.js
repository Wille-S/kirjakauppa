...
function kaynnistaPalvelin(asetustiedosto){
    ...
    const palvelin = http.createServer(async (req,res)=>{
        console.log('New request:', req.method, req.url);  // Log the incoming request method and URL
        const {pathname} = new URL(`http://${req.headers.host}${req.url}`);
        const reitti=decodeURIComponent(pathname);

        try{
            const metodi=req.method.toUpperCase();
            console.log('Processing method:', metodi);
            if(metodi==='OPTIONS'){
                ...
                if(metodi==='GET'){
                    ...
                    try{
                        ...
                        console.log('Parsed JSON:', tulos);
                        lahetaJson(res, await tietovarasto.lisaa(tulos));
                    }
                    catch(virhe){
                        console.log('Error in POST:', virhe);  // Log any errors that occur during POST
            ...
            console.log('General server error:', virhe);  // Log general server errors
        }
